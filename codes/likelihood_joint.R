...
likelihood_joint <- function(lambda, chi_BASE, chi_BEL){
  ...
  sigma_A=solve_qre(D = D_BASE, R = R_BASE_A, M = M_BASE, lambda, chi_BASE)
  sigma_B=solve_qre(D = D_BASE, R = R_BASE_B, M = M_BASE, lambda, chi_BASE)
  ...
  treat=(treatment=="BEL")
  sigma_A=1/(1/exp(lambda*(exp_match_pay(S_BEL_A,D_BEL,M_BEL,chi_BEL) - R_BEL))+1)
  sigma_B=1/(1/exp(lambda*(exp_match_pay(S_BEL_B,D_BEL,M_BEL,chi_BEL) - R_BEL))+1)
  sigma_C=1/(1/exp(lambda*(exp_match_pay(S_BEL_C,D_BEL,M_BEL,chi_BEL) - R_BEL))+1)
  sigma_D=1/(1/exp(lambda*(exp_match_pay(S_BEL_D,D_BEL,M_BEL,chi_BEL) - R_BEL))+1)
  sigma_E=1/(1/exp(lambda*(exp_match_pay(S_BEL_E,D_BEL,M_BEL,chi_BEL) - R_BEL))+1)
